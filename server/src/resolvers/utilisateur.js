...
const getTokenTypeFromUser = (user) =>{
	return {
		id: user.id,
		role: user.role,
		email: user.email,
...
/**
 * Génère un Web Token pour l'utilisateur.
...
module.exports = {
  ...
  Mutation: {
    ...
    async connexion(_, { email, motDePasse, role }, { prisma }) {
      ...
      const { errors, valid } = validateLoginInput(email, motDePasse, role);
      ...
      if (!user) {
        ...
        return new UserInputError("Adresse mail ou mot de passe incorrecte", {
          errors,
        });
      }
	...
	const tokUser = getTokenTypeFromUser(user)
    ...
    async inscription(_, { inscriptionInput }, { prisma }) {
      ...
      if (userExists) {
        return new UserInputError("Adresse mail deja utilisée pour ce role", {
          errors: {
      ...
      if (role === ETUDIANT) {
        ...
        if (studentExists) {
          return new UserInputError(
            "Vous êtes deja enregistré avec ce numero matricule",
            {
              errors: {
                id: "Ce numéro matricule est deja associé à un étudiant.",
              },
            }
          );
        }
    ...
    async updateUser(_, { updateUserInput }, context) {
      ...
      if (ancienMotDePasse) {
        ...
        const match = await bcrypt.compare(ancienMotDePasse, upUser.motDePasse);
        if (!match) {
          errors.general = "Mot de passe incorrect";
          return new UserInputError("L'ancien mot de passe est incorrect", {
            errors,
          });
        }
        // Hash du nouveau mot de passe
        newUsVal.motDePasse = await bcrypt.hash(newUsVal.motDePasse, 12);
      }
      ...
      if (Object.keys(newUsVal).length > 0) {
        try {
          ...
          if (
            ...
            return new UserInputError("Adresse mail deja utilisée", {
              errors: {
                email: "Cette adresse mail est déjà utilisée pour ce rôle. Veuillez en choisir une autre",
              },
            });
      ...
      if (upUser) {
		  const tokUser = getTokenTypeFromUser(upUser)
		  const token = generateToken(tokUser);
