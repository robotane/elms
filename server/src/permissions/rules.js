...
module.exports = {
    ...
    isContEdOwner: rule()(async (_parent, { idContenuEducatif }, context) => {
        ...
        const contEd = await prisma.contenuEducatif.findUnique({
            ...
            include: {
                enseignants: {
                    where: {
                        id: Number[user.idProp],
                    },
                },
            },
        });

        const ens = contEd.enseignants.find((e) => e.id === user.idProp);
        return Boolean(ens);
    }),
    ...
    isCommentOwner: rule()(async (_par, { idCommentaire }, context) => {
        const { prisma } = context;
        const user = checkAuth(context);
        const hasComment = await prisma.utilisateur.findUnique({
        ...
        return hasComment.commentaires.length > 0;
    }),
};
